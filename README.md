...
```text
.
├── runtime.go        # public runtime orchestration
├── config.go         # public config aliases and defaults
├── interfaces.go     # public adapter interfaces
├── result.go         # public result aliases
├── core/             # shared domain types
├── task/             # mutable task context and counters
├── planner/          # plan generation and parsing
├── plan/             # plan management and validation
├── executor/         # ReAct step execution
├── tool/             # registry, gateway wrapper, policy, schema checks
├── audit/            # periodic execution audit
├── reflection/       # failure analysis
├── correction/       # plan correction
├── experience/       # experience provider helpers
├── hook/             # hook manager
├── exit/             # exit decision logic
└── docs/             # design and development prompt documents
```
...
If your local Go build cache is writable, plain `go test ./...` is also fine.

## Development Guidelines

- Keep the root package small and stable; expose implementation details through
  aliases only when they are part of the embedding contract.
- Prefer adding behavior behind existing interfaces instead of introducing
  service-specific dependencies.
- Keep model output parsing strict and covered by tests.
- Keep tool execution outside this library. This runtime should decide,
  validate, observe, and record; the host application should perform side
  effects.
- Add tests around every new exit path, policy decision, parser shape, and
  config mutation.
